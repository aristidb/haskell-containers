...
removeDuplicates (SortedAList xs) = SortedAList $ foldr removeDuplicate [] xs
    where removeDuplicate x@(k, _) ys@(y@(k', _) : _)
              | k == k' = ys
              | otherwise = x : ys
          removeDuplicate x _ = [x]

removeDuplicates' (SortedAList xs) = SortedAList $ foldr removeDuplicate [] xs
    where removeDuplicate x@(k, _) ys@(y@(k', _) : ys')
              | k == k' = x : ys'
              | otherwise = x : ys
          removeDuplicate x _ = [x]
